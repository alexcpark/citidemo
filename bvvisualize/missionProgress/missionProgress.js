...
BVVIZ.missionProgress = function( target, playerId, missionId, options ) {
  ...
  function init() {
    ...
    BVSDK.all(
      BVVIZ.SDKkit.getInstanceBySiteId( settings.siteId ).BVSDK( 'missions', { missions: missionId }, { fields: 'hint' } ),
      BVVIZ.SDKkit.getInstanceBySiteId( settings.siteId ).BVSDK( 'players/missions', { players: playerId, missions: missionId }, { fields: 'hint' } )
