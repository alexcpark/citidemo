...
BVVIZ.leaderboard = function( target, leaderboardIds, options ) {
  ...
  function init() {
    ...
    $.each( leaderboardIds, function( i, id ) {
      lbPromises.push(
        BVVIZ.SDKkit.getInstanceBySiteId( settings.siteId ).BVSDK( 'leaderboards', { leaderboards: id } )
      );
    });
    ...
    if ( currentLB === board ) {
      ...
      newCards.fadeIn( null, function() {
        ...
        if ( settings.loadLBToggle && BVVIZ.currentPlayer && BVVIZ.currentPlayer.id ) {
          BVVIZ.SDKkit.getInstanceBySiteId( settings.siteId ).BVSDK( 'players/positions', { players: BVVIZ.currentPlayer.id, positions: leaderboardIds[ currentLB ] }, {
            fields: 'all'
          }).ok( function( data ) {
  ...
  function load( offset, where ) {
    ...
    BVVIZ.SDKkit.getInstanceBySiteId( settings.siteId ).BVSDK( 'leaderboards/ranks', { leaderboards: leaderboardIds[ currentLB ] }, {
      offset: offset,
      limit: limit,
      fields: 'all'
    }).ok( function( data ) {
  ...
  function loadPlayer( card ) {
    ...
    BVVIZ.SDKkit.getInstanceBySiteId( settings.siteId ).BVSDK( 'players', { players: card.data( 'bvviz_type_id' ) }, {
      fields: 'all',
  ...
  function loadTeam( card ) {
    ...
    card.append( BVVIZ.helper.loading() );
    // Load the team data
    BVVIZ.SDKkit.getInstanceBySiteId( settings.siteId ).BVSDK( 'teams', { teams: card.data( 'bvviz_type_id' ) }, {
      fields: 'all'
    }).ok( function( data ) {
      // Remove the loading icon
      $( '.bvviz-loading', card ).remove();
