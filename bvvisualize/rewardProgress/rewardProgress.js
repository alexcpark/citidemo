...
BVVIZ.rewardProgress = function( target, playerId, rewardId, options ) {
  ...
  function init() {
    ...
    BVSDK.all(
      ...
      BVVIZ.SDKkit.getInstanceBySiteId( settings.siteId ).BVSDK( 'rewards', null, {
        fields: 'all',
        query: { definition_id: rewardId }
      } ),      
      // Load the reward information and player's overall reward progress
      BVVIZ.SDKkit.getInstanceBySiteId( settings.siteId ).BVSDK( 'players/rewards', { players: playerId }, {
        fields: 'all',
