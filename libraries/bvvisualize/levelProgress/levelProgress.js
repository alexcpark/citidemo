...
BVVIZ.levelProgress = function( target, playerId, missionId, options ) {
  ...
  function init() {
    ...
    BVSDK( 'players', { players: playerId }, { includes:'rewards:currentlevels' } )
      .ok( function( data ) {
                    ...
                    .ok( function ( missionInfo ) {
                      ...
                      try {
                        if ( $.isPlainObject( criteriaObj ) ) {
                          ...
                          if ( missionInfo.rewards.length == 2 ) {
                            // Both current level and next level found
                            target.append( render.card( missionInfo.rewards[0], missionInfo.rewards[1], playerEarnedTotal ) );
                          } else {
                            // Could happen when the current reward is the last level
                            target.append( render.card( missionInfo.rewards[0], null, playerEarnedTotal ) );
                          }
                        }
