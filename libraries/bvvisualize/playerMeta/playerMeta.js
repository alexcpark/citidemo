...
;(function($) {
// Add playerMeta method to the BVVIZ scope
BVVIZ.playerMeta = function( target, player, options ) {
  ...
  settings = $.extend( {}, BVVIZ.options.playerMeta, options ),

  // Store the unitBar settings for playerCard
  unitBarSettings = $.extend( { visualization : 'playerMeta' }, settings.unitBar ),
  ...
  function show() {
    ...
    if ( $.isPlainObject( player ) ) {
      ...
      BVSDK(
        'players',
        { players: player },
        { includes: inclusions, fields: 'all' }
      ).ok( function( data ) {
